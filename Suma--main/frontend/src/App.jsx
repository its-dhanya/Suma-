...
// ── Capture & OCR controls (Capture tab) ───────────────────────────────────
function CaptureControls({ setTranscript }) {
...
function App() {
  ...
  // Poll session status to survive reloads and keep state in sync
  useEffect(() => {
    const checkStatus = async () => {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          setIsRecording(data.audio_alive || data.screen_alive);
  ...
  const startSession = async (mode = "esp32") => {
    setLoading((prev) => ({ ...prev, session: true }));
    setIsRecording(true);
    try {
      const res  = await fetch(`http://localhost:8000/start-session?mode=${mode}`, { method: "POST" });
      const data = await res.json();
      setSessionFolder(data.session_folder);
    } catch (error) {
  ...
  const exportPDF = () => {
    ...
    const sections = [
      ...
      { title: "Audio Summary",           color: [50,  130, 220], content: formatContent(summary.audio_content_summary) },
      { title: "Board Summary (OCR)",     color: [60,  170, 110], content: formatContent(summary.board_content_summary) },
      { title: "Core Concepts",           color: [120,  90, 255], content: formatContent(summary.core_concepts) },
      { title: "Key Points",              color: [220, 140,  60], content: formatContent(summary.key_points || summary.takeaways) },
      { title: "Examples",                color: [70,  180, 120], content: formatContent(summary.examples) },
      { title: "Applications",            color: [50,  180, 180], content: formatContent(summary.applications) },
      { title: "Revision Questions",      color: [200,  80,  80], content: formatContent(summary.questions) },
      { title: "Final Recap",             color: [80,   80,  80], content: formatContent(summary.summary) },
      ...
      <nav className="w-full max-w-5xl mb-6">
        <ul className="flex justify-around bg-white/90 backdrop-blur-md rounded-full p-3 shadow-md">
          {[
            ...
            { id: "capture",    label: "Capture",    active: "bg-indigo-600" },
            { id: "transcript", label: "Transcript", active: "bg-green-600" },
            { id: "alignment",  label: "Alignment",  active: "bg-red-600" },
      ...
      <div className="w-full max-w-5xl bg-white/90 backdrop-blur-lg rounded-3xl shadow-2xl border border-gray-300 p-10 space-y-8">
        ...
        {activeTab === "home" && (
          <div className="text-center space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-8">
              {[
                { title: "Record",           body: "Record your live classroom sessions easily." },
        ...
        {activeTab === "capture" && (
          <div className="space-y-6">
            ...
            />
            <CaptureControls setTranscript={setTranscript} />
          </div>
        )}
        ...
        {activeTab === "transcript" && (
          <div className="space-y-6">
            <TranscriptView transcript={transcript} sessionFolder={sessionFolder} />
          </div>
        )}
        ...
        {/* ── ALIGNMENT ── */}
        {activeTab === "alignment" && (
          <AlignmentView sessionFolder={sessionFolder} />
        )}
      </div>
